...
describe('chunk traceability', () => {
    it('keeps chunk metadata traceable to the source document and section', async () => {
        ...
        expect(chunk?.sectionPathKey).toBe(section?.pathKey);
        expect(chunk?.citationMetadataJson).toMatchObject({
            sourceUrl: sample.sourceUrl,
            sourceName: sample.sourceName,
